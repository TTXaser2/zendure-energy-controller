...
CONTROL_SNAPSHOT_KEYS = [
    ...
    "REST_SURPLUS_HARVEST_ENABLED", "HARVEST_FAST_CAPTURE_MODE", "SECOND_BATTERY_MAX_CHARGE_POWER_W",
    ...
    "HARVEST_SMA_FULL_SOC_PERCENT", "HARVEST_PRIMARY_CHARGE_FLOOR_RATIO",
    "HARVEST_PRIMARY_CHARGE_FLOOR_W", "HARVEST_PRIMARY_CHARGE_RESTART_RATIO",
    "HARVEST_PRIMARY_CHARGE_RESTART_W", "HARVEST_PRIMARY_CHARGE_NEAR_LIMIT_RATIO",
    "HARVEST_PRIMARY_CHARGE_NEAR_LIMIT_W", "HARVEST_HIGH_SMA_SOC_TIME_PROFILE_ENABLED",
    "HARVEST_PRIMARY_CHARGE_TARGET_SHARE_MORNING", "HARVEST_PRIMARY_CHARGE_TARGET_SHARE_MIDDAY",
    ...
    "SMA_ENERGY_METER_SOCKET_MODE", "SMA_ENERGY_METER_PACKET_GAP_WARN_SECONDS",
]
...
def build_v4_row(config: Dict[str, Any], row: Dict[str, Any], previous_effective_command_w: Optional[float] = None) -> Dict[str, Any]:
    ...
    v4 = {
        ...
        "primary_discharge_floor_supported": _bool01(row.get("primary_discharge_floor_supported")),
        "primary_discharge_floor_soc_percent": _round1(row.get("primary_discharge_floor_soc_percent")),
        "primary_discharge_floor_valid": _bool01(row.get("primary_discharge_floor_valid")),
        "primary_discharge_floor_fresh": _bool01(row.get("primary_discharge_floor_fresh")),
        "primary_discharge_floor_age_s": _round1(row.get("primary_discharge_floor_age_s")),
        "primary_discharge_floor_source": str(row.get("primary_discharge_floor_source") or ""),
        "primary_usable_soc_percent": _round1(row.get("primary_usable_soc_percent")),
        "primary_usable_soc_valid": _bool01(row.get("primary_usable_soc_valid")),
        ...
        "rest_surplus_harvest_reason": str(row.get("rest_surplus_harvest_reason", "NONE") or "NONE"),
        "rest_surplus_harvest_block_reason": str(row.get("rest_surplus_harvest_block_reason", "") or ""),
        "rest_surplus_harvest_profile": str(row.get("rest_surplus_harvest_profile", "default") or "default"),
        "rest_surplus_entry_progress_s": _round1(row.get("rest_surplus_entry_progress_s")),
        "rest_surplus_hold_remaining_s": _round1(row.get("rest_surplus_hold_remaining_s")),
        ...
        "harvest_primary_floor_w": _round1(row.get("harvest_primary_floor_w")),
        "harvest_primary_restart_w": _round1(row.get("harvest_primary_restart_w")),
        "harvest_primary_near_limit_w": _round1(row.get("harvest_primary_near_limit_w")),
        "harvest_primary_target_share": _round1(row.get("harvest_primary_target_share")),
        "harvest_primary_required_w": _round1(row.get("harvest_primary_required_w")),
        ...
        "fast_capture_mode": str(row.get("fast_capture_mode", "off") or "off"),
        "fast_capture_active": _bool01(row.get("fast_capture_active")),
        "fast_capture_primary_state": str(row.get("fast_capture_primary_state", "RESERVE_UNKNOWN") or "RESERVE_UNKNOWN"),
        "fast_capture_block_reason": str(row.get("fast_capture_block_reason", "") or ""),
        "fast_capture_baseline_target_w": _round1(row.get("fast_capture_baseline_target_w")),
        "fast_capture_desired_overlay_w": _round1(row.get("fast_capture_desired_overlay_w")),
        "fast_capture_overlay_w": _round1(row.get("fast_capture_overlay_w")),
        "fast_capture_combined_target_w": _round1(row.get("fast_capture_combined_target_w")),
        "fast_capture_primary_reserve_w": _round1(row.get("fast_capture_primary_reserve_w")),
        "fast_capture_full_idle_progress_s": _round1(row.get("fast_capture_full_idle_progress_s")),
        "fast_capture_observation_dt_s": _round1(row.get("fast_capture_observation_dt_s")),
        "fast_capture_observation_distinct": _bool01(row.get("fast_capture_observation_distinct")),
        "fast_capture_attack_limited": _bool01(row.get("fast_capture_attack_limited")),
        "fast_capture_release_limited": _bool01(row.get("fast_capture_release_limited")),
        "fast_capture_forced_zero_reason": str(row.get("fast_capture_forced_zero_reason", "") or ""),
        "fast_capture_activation_count": _safe_int(row.get("fast_capture_activation_count")) or 0,
        "fast_capture_forced_zero_count": _safe_int(row.get("fast_capture_forced_zero_count")) or 0,
        "fast_capture_effective_zendure_limit_w": _round1(row.get("fast_capture_effective_zendure_limit_w")),
        "fast_capture_primary_max_charge_w": _round1(row.get("fast_capture_primary_max_charge_w")),
