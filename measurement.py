...
def derive_zendure_actual_power(
    ...
    Sign convention:
    - gridInputPower / packInputPower candidates count as positive charging.
    - outputHomePower / outputPackPower candidates count as negative discharging.

    The raw sensor values are kept separately in state/CSV. This function only
    derives the signed display and analysis values.
    """
    ...
    for value in (gi, pi):
        if value is not None:
            charge_candidates.append(max(0, value))
    for value in (oh, op):
        if value is not None:
