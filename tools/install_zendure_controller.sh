...
# Canonical ZEC V17 installer: supported update + clean fresh install.
set -Eeuo pipefail
...
INSTALL_LOG="$DOWNLOAD_DIR/zec_${VERSION}_installer_${STAMP}.log"
if ! touch "$INSTALL_LOG" 2>/dev/null; then
  INSTALL_LOG="/tmp/zec_${VERSION}_installer_${STAMP}.log"
  touch "$INSTALL_LOG"
fi
...
CONFIG_BACKUP="$DOWNLOAD_DIR/config.pre-v17.0.1.${STAMP}.json"
ROOT_ARTIFACT_BACKUP="/var/backups/zec-v17.0.1-root-artifacts-${STAMP}"
INSTALL_REPORT="$DOWNLOAD_DIR/zec_${VERSION}_install_report_${STAMP}.json"
INSTALL_REPORT_COMPAT="/tmp/zec_${VERSION}_install_report.json"
BACKUP_SHA256=""
BACKUP_SIZE="0"
...
start_support_capture() {
  ...
  SUPPORT_WORK="$(python3 "$tool" collect \
    --label "v17.0.1-installer-failure" --output-dir "$DOWNLOAD_DIR" \
...
on_error() {
  ...
  set +e
  echo
  echo "FEHLER: V17.0.1 Installer abgebrochen (rc=$code, line=$line, mode=$INSTALL_MODE)."
...
require_basic_contract() {
  ...
  [ -w "$DOWNLOAD_DIR" ] || { echo "FEHLER: $DOWNLOAD_DIR ist nicht beschreibbar; persistente Installations-/Rollback-Evidenz kann nicht erzeugt werden."; return 1; }
}
...
verify_build_evidence_at() {
  ...
  PYTHONDONTWRITEBYTECODE=1 python3 "$root/tools/deployment_contract.py" verify-build-evidence \
    --root "$root" --evidence "validation/V17_0_1_BUILD_EVIDENCE.json" \
    --expected-version "$EXPECTED_TARGET_VERSION" --expected-label "V17.0.1" \
    --expected-build "$EXPECTED_TARGET_BUILD_ID" --json >/dev/null
  echo "Build-Testevidenz für V17.0.1 verifiziert."
}
...
if [ "$INSTALL_MODE" = "SUPPORTED_UPDATE" ]; then
  ...
  echo "Kopiere V17.0.1-Dateien; Benutzerdaten bleiben erhalten..."
  rsync -a --delete \
  ...
  echo "Erzeuge saubere V17.0.1-Fresh-Installation ohne config.json..."
...
PY
  ...
  if [ "$READY_OK" -eq 1 ]; then
    ...
    echo "FEHLER: V17.0.1 erreichte weder ready=true noch einen stabilen sicheren Übergangszustand."
    [ -s "$READY_JSON" ] && cat "$READY_JSON"
    false
  fi
...
python3 - "$INSTALL_REPORT" "$INSTALL_REPORT_COMPAT" <<PY
...
atomic_write(sys.argv[1])
try:
    atomic_write(sys.argv[2])
except OSError as exc:
    print(f'WARNUNG: /tmp-Kompatibilitätskopie des Installationsreports konnte nicht geschrieben werden: {exc}',file=sys.stderr)
PY

trap - ERR EXIT
cleanup_stage
echo "V17.0.1 erfolgreich installiert."
...
echo "Persistenter Installationsreport: $INSTALL_REPORT"
echo "Kompatibilitätskopie Installationsreport: $INSTALL_REPORT_COMPAT"
echo "Persistentes Installerlog: $INSTALL_LOG"
