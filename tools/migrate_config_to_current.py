...
"""Fail-closed migration of a persisted ZEC config to the current runtime schema.

Uses the shared config_migration authority also used by import/load flows. Unknown
keys are preserved; no value is clamped or silently repaired. A write happens
only after full current-registry validation and is verified byte-for-byte.
"""
...
from config_migration import migrate_to_current  # noqa: E402
...
def parser() -> argparse.ArgumentParser:
    value = argparse.ArgumentParser(description="ZEC config migration to current schema")
...
def result_payload(*, path: Path, before_revision: str, after_revision: str, changed: bool, steps: tuple[str, ...]) -> Dict[str, Any]:
    return {
        "schema": "ZEC-CONFIG-MIGRATION-CURRENT-V1",
...
def main() -> int:
    ...
    try:
        migration = migrate_to_current(raw)
    except ValueError as exc:
        raise SystemExit("CONFIG_MIGRATION_FAILED:" + str(exc)) from None
    candidate = parse_full_candidate(dict(migration.configured), previous=raw)
