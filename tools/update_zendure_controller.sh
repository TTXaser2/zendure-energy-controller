...
EXPECTED_TARGET_BUILD_ID="v12.12.2-20260810"

if [ "$VERSION" != "$EXPECTED_VERSION" ]; then
    echo "FEHLER: Dieses Update-Skript unterstützt V12.12.1 sowie die dokumentierten kompatiblen Recovery-Ausgangsstände als Quelle für V12.12.2."
...
CONFIG_BACKUP="/home/pi/config.pre-v12.12.2.${STAMP}.json"
ROOT_ARTIFACT_BACKUP="/var/backups/zec-v12.12.2-root-artifacts-${STAMP}"
...
recover_on_error() {
    ...
    if [ "$INSTALLATION_STARTED" -eq 0 ]; then
        echo "FEHLER: V12.12.2-Paketvorprüfung wurde abgebrochen."
        echo "Die Produktivinstallation wurde noch nicht begonnen; Dienste und /opt/zendure-controller blieben unverändert."
        exit "$exit_code"
    fi
    echo "FEHLER: V12.12.2-Update wurde während der Produktivinstallation abgebrochen. Starte automatischen Rollback."
...
verify_source_manifest() {
    [ -f "$DIR/V12_12_2_SOURCE_MANIFEST.sha256" ] || {
        echo "FEHLER: V12_12_2_SOURCE_MANIFEST.sha256 fehlt im Paket."
        return 1
    }
    (
        trap - ERR
        cd "$DIR"
        sha256sum -c V12_12_2_SOURCE_MANIFEST.sha256 >/dev/null
    )
}
...
if [ "$INSTALLED_VERSION" = "$EXPECTED_SOURCE_RC19" ]; then
    ...
    echo "Erlaubt sind exakt V12.12.1, V12.12.0, V12.11.7, V12.11.6, V12.11.5, V12.11.4, V12.11.3, RC20 Fix 6, RC20 Fix 5 oder RC19."
    exit 1
fi
...
echo "V12.12.2-Paket vor dem Stoppen des Produktivdienstes entpacken und prüfen..."
rm -rf "$DIR"
...
[ "$TARGET_PACKAGE_VERSION" = "12.12.2" ] || { echo "FEHLER: Paket meldet Version ${TARGET_PACKAGE_VERSION}"; exit 1; }
...
echo "Kopiere V12.12.2-Dateien; config.json, Last-Good und Laufzeitdaten bleiben erhalten..."
rsync -a \
  --exclude 'config.json' \
...
if [ "$READY_OK" -eq 1 ]; then
    ...
    echo "FEHLER: V12.12.2 erreichte weder ready=true noch einen stabilen sicheren Übergangszustand."
    [ -s "$READY_JSON" ] && cat "$READY_JSON"
...
echo "V12.12.2 erfolgreich installiert."
echo "Backup: $BACKUP"
