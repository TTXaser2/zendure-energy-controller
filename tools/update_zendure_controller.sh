...
EXPECTED_TARGET_BUILD_ID="v12.11.5-20260807"

if [ "$VERSION" != "$EXPECTED_VERSION" ]; then
    echo "FEHLER: Dieses Update-Skript unterstützt V12.11.4 sowie die dokumentierten kompatiblen Recovery-Ausgangsstände als Quelle für V12.11.5."
...
CONFIG_BACKUP="/home/pi/config.pre-v12.11.5.${STAMP}.json"
ROOT_ARTIFACT_BACKUP="/var/backups/zec-v12.11.5-root-artifacts-${STAMP}"
...
recover_on_error() {
    ...
    if [ "$INSTALLATION_STARTED" -eq 0 ]; then
        echo "FEHLER: V12.11.5-Paketvorprüfung wurde abgebrochen."
        echo "Die Produktivinstallation wurde noch nicht begonnen; Dienste und /opt/zendure-controller blieben unverändert."
        exit "$exit_code"
    fi
    echo "FEHLER: V12.11.5-Update wurde während der Produktivinstallation abgebrochen. Starte automatischen Rollback."
...
verify_source_manifest() {
    [ -f "$DIR/V12_11_5_SOURCE_MANIFEST.sha256" ] || {
        echo "FEHLER: V12_11_5_SOURCE_MANIFEST.sha256 fehlt im Paket."
        return 1
    }
    (
        trap - ERR
        cd "$DIR"
        sha256sum -c V12_11_5_SOURCE_MANIFEST.sha256 >/dev/null
    )
}
...
if [ "$INSTALLED_VERSION" = "$EXPECTED_SOURCE_RC19" ]; then
    ...
    echo "Erlaubt sind exakt V12.11.4, V12.11.3, RC20 Fix 6, RC20 Fix 5 oder RC19."
    exit 1
fi
...
echo "V12.11.5-Paket vor dem Stoppen des Produktivdienstes entpacken und prüfen..."
rm -rf "$DIR"
...
[ "$TARGET_PACKAGE_VERSION" = "12.11.5" ] || { echo "FEHLER: Paket meldet Version ${TARGET_PACKAGE_VERSION}"; exit 1; }
...
echo "Kopiere V12.11.5-Dateien; config.json, Last-Good und Laufzeitdaten bleiben erhalten..."
rsync -a \
  --exclude 'config.json' \
...
if [ "$READY_OK" -eq 1 ]; then
    ...
    echo "FEHLER: V12.11.5 erreichte weder ready=true noch einen stabilen sicheren Übergangszustand."
    [ -s "$READY_JSON" ] && cat "$READY_JSON"
...
echo "V12.11.5 erfolgreich installiert."
echo "Backup: $BACKUP"
