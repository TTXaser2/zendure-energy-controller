...
CONFIG_SCHEMA: Dict[str, Dict[str, Any]] = {
    ...
    "SMA_ENERGY_METER_SOCKET_MODE": {"group": "Netzwerk", "label": "SMA Socket-Modus", "type": "select", "options": {"group_bind": "Empfohlen: Bind auf Multicast-Gruppe", "rc3_compatible": "Diagnose: Wildcard-Bind + SO_REUSEPORT best-effort", "reuseaddr_only": "Diagnose: nur SO_REUSEADDR", "unimeter_like": "Diagnose: UniMeter-naher Join ohne IP_MULTICAST_IF"}, "description": "Experten-/Diagnoseoption für die Koexistenz mit EVCC, UniMeter oder weiteren SMA-Speedwire-Listenern auf demselben Host. Default group_bind bindet auf die SMA-Multicast-Gruppe und war im Nachtlauf mit EVCC auf demselben Raspberry Pi stabil. Wildcard-Modi auf 0.0.0.0:9522 bleiben reine Diagnoseoptionen und können EVCC stören."},
    ...
    "ZENDURE_LOCAL_API_CONTROL_TIMEOUT_CAP_SECONDS": {"group": "Netzwerk", "label": "Zendure lokale API Request-Timeoutdeckel", "type": "float", "min": 0.2, "max": 5.0, "unit": "s", "description": "Begrenzt den Hintergrundrequest unabhängig vom höheren allgemeinen API-Timeout. Die lokale API arbeitet asynchron und blockiert den Regelzyklus nicht."},
    ...
    "SLOW_CYCLE_WARN_MS": {"group": "Regelung", "label": "Warnschwelle langsamer Zyklus", "type": "int", "min": 1000, "max": 60000, "unit": "ms", "description": "Schreibt einen Runtime-Hinweis, wenn ein Reglerzyklus ohne Sleep länger dauert. Dient der Laufzeit- und Timingdiagnose."},
    ...
    "COMMAND_RESYNC_ON_MQTT_RECOVERY_ALWAYS": {"group": "Regelung", "label": "Zendure-Command-Resync bei jeder MQTT-Recovery", "type": "bool", "description": "Notfall-/Legacy-Schalter: erzwingt Resync bei jeder Zendure-MQTT-Recovery. Standardmäßig ist die Option deaktiviert, damit kurze STALE-Phasen keinen unnötigen Resync auslösen."},
    ...
    "COMMAND_EFFECT_TOLERANCE_W": {"group": "Regelung", "label": "Absolute Soll/Ist-Toleranz für Zielwerttracking", "type": "int", "min": 10, "max": 500, "unit": "W", "description": "Absolute Mindesttoleranz für das Zielwerttracking. Verwendet wird der größere Wert aus dieser absoluten Toleranz und der relativen Prozenttoleranz."},
    "COMMAND_EFFECT_TOLERANCE_PERCENT": {"group": "Regelung", "label": "Relative Soll/Ist-Toleranz für Zielwerttracking", "type": "int", "min": 0, "max": 30, "unit": "%", "description": "Relative Toleranz für hohe Sollwerte. Sie verändert nur die Wirkungsdiagnose, niemals den Regler-Sollwert oder eine Gerätegrenze."},
    ...
    "HARVEST_FAST_CAPTURE_MODE": {"group": "Harvest / Restüberschuss", "subgroup": "Fast Capture", "label": "Fast Capture", "type": "select", "options": {"off": "Aus", "shadow": "Shadow (nur Diagnose)", "active": "Aktiv"}, "description": "A400/R100 Fast-Capture-Overlay für bestätigte FULL_IDLE-/NEAR_LIMIT-Situationen. Shadow berechnet und protokolliert ohne Sollwertwirkung; Aktiv darf nur mit vollständigen Harvest-/Cross-Charge-/Primärspeicher-Voraussetzungen verwendet werden."},
    ...
    "MEASUREMENT_SCHEMA_VERSION": {"group": "Messdaten / Historie", "label": "Measurement-Schema", "type": "select", "options": {"4": "ZEC-MEASUREMENT-V4"}, "description": "Fester, nicht editierbarer Kompatibilitätsmarker. Produktives Measurement wird ausschließlich als V4 geschrieben; historische V3-Dateien bleiben nur offline lesbar."},
